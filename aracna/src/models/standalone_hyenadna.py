...
class HyenaOperator(nn.Module):
    def __init__(
        ...
        **filter_args,
        ...
        if not self.causal:
            self.backwards_filter = copy.deepcopy(self.short_filter)
            self.backwards_filter_fn = copy.deepcopy(self.filter_fn)

    def main_step(self, filter, filter_func, input_seq, l_filter):
        uc = filter(input_seq)[..., :l_filter]
        *x, v = uc.split(self.d_model, dim=1)

        k = filter_func.filter(l_filter)[0]
        k = rearrange(k, "l (o d) -> o d l", o=self.order - 1)
        bias = rearrange(filter_func.bias, "(o d) -> o d", o=self.order - 1)

        for o, x_i in enumerate(reversed(x[1:])):
            v = self.dropout(v * x_i)
            v = filter_func(v, l_filter, k=k[o], bias=bias[o])
    ...
    def forward(self, u, *args, **kwargs):
        ...
        y = self.main_step(self.short_filter, self.filter_fn, u, l_filter)

        if not self.causal:
            y_backwards = self.main_step(
                self.backwards_filter, self.backwards_filter_fn, u.flip([-1]), l_filter
            ).flip([-1])
            y = y + y_backwards
