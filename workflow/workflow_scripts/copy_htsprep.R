...
ascat.try_getAlleleCounts = function(seq.file, output.file, loci.file, min.base.qual=20, min.map.qual=35, allelecounter.exe="alleleCounter", ref.fasta=NA) {
...
#' @author dw9, sd11, tl
#' @export
ascat.try_getBAFsAndLogRs = function(samplename, tumourAlleleCountsFile.prefix, normalAlleleCountsFile.prefix, tumourLogR_file, tumourBAF_file, normalLogR_file, normalBAF_file, alleles.prefix, gender, genomeVersion, chrom_names=c(1:22, "X"), minCounts=20, BED_file=NA, probloci_file=NA, seed=as.integer(Sys.time())) {
  set.seed(seed)
  ...
  if (gender=="XY") {
    ...
    # nonPAR should be X-specific and baseline is 1+0 so logR needs to be decreased according to gamma parameter (ascat.try_runAscat)
...
#' @author tl
#' @export
ascat.try_synchroniseFiles=function(samplename, tumourLogR_file, tumourBAF_file, normalLogR_file, normalBAF_file) {
  # read all files
...
#' @author sd11, tl
#' @export
ascat.try_prepareHTS = function(tumourseqfile, normalseqfile, tumourname, normalname, allelecounter_exe, alleles.prefix, loci.prefix, gender, genomeVersion,
  ...
  print(skip_allele_counting_tumour)
  print(skip_allele_counting_normal)
  ...
  if (!skip_allele_counting_tumour) {
    ...
    foreach(CHR=chrom_names) %dopar% {
      CHR=get("CHR")
      print(paste0(loci.prefix, CHR, ".txt"))
  ...
  if (!skip_allele_counting_normal) {
    ...
    print("should be done 2")
  }

  # Obtain BAF and LogR from the raw allele counts
  ascat.try_getBAFsAndLogRs(samplename=tumourname,
